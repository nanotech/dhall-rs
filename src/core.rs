...
impl<'i, S, A: Display> Expr<'i, S, A> {
    ...
    fn fmt_c(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        ...
        match self {
            ...
            &TextAppend(ref a, ref b) => {
                a.fmt_d(f)?;
                f.write_str(" ++ ")?;
                b.fmt_c(f)
            }
            &NaturalPlus(ref a, ref b) => {
                a.fmt_d(f)?;
                f.write_str(" + ")?;
                b.fmt_c(f)
            }
    ...
    fn fmt_f(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        ...
        match self {
            ...
            &Record(ref a) => fmt_list("{ ", " }", a, f, |(k, t), f| write!(f, "{} : {}", k, t)),
            &RecordLit(ref a) if a.is_empty() => f.write_str("{=}"),
            &RecordLit(ref a) => fmt_list("{ ", " }", a, f, |(k, v), f| write!(f, "{} = {}", k, v)),
    ...
    A: Clone + fmt::Debug,
{
    ...
    match *e {
        ...
        BuiltinValue(v) => BuiltinValue(v),
        BoolLit(b) => BoolLit(b),
        BoolAnd(ref x, ref y) => with_binop(
            BoolAnd,
            Expr::bool_lit,
            |xn, yn| BoolLit(xn && yn),
            normalize(x),
        ...
        NaturalPlus(ref x, ref y) => with_binop(
            NaturalPlus,
            Expr::natural_lit,
        ...
        NaturalTimes(ref x, ref y) => with_binop(
            NaturalTimes,
            Expr::natural_lit,
            |xn, yn| NaturalLit(xn * yn),
            normalize(x),
        ...
        TextAppend(ref x, ref y) => with_binop(
            TextAppend,
            Expr::text_lit,
            |xt, yt| TextLit(xt + &yt),
            normalize(x),
